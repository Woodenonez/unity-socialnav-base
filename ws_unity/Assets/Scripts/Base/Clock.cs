...
/// Provide a single source of truth for time from Unity.
/// Major interfaces:
/// - Clock.Now: current time in seconds (double)
/// - Clock.time: time at the start of the current frame in seconds (double)
/// - Clock.deltaTime: time since last frame in seconds (float)
/// Usage:
...
namespace Unity.Robotics.Core
{
    ...
    {
        // UnityScaled is the default Time mode. None of the other time modes are guaranteed to work
